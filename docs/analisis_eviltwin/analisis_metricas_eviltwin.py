# =============================================================================
#   ANÁLISIS DE MÉTRICAS IDS EVIL TWIN — Para la memoria del TFG
# =============================================================================
...
CSV_PATH = "/Users/joseda_cond/Desktop/- TFG -/logs/log_ids_eviltwin_ml_20260401_233546.csv"
OUTPUT_DIR = "/Users/joseda_cond/Desktop/- TFG -/tfg-wifi-ids-iot-ml/docs/analisis"
...
print("\n" + "="*60)
print("  ANÁLISIS DE MÉTRICAS — IDS EVIL TWIN")
print("="*60)
...
tp = ((df['ground_truth'] == 1) & (df['alerta'] == 1)).sum()
fn = ((df['ground_truth'] == 1) & (df['alerta'] == 0)).sum()
fp = ((df['ground_truth'] == 0) & (df['alerta'] == 1)).sum()
tn = ((df['ground_truth'] == 0) & (df['alerta'] == 0)).sum()
...
# Fondo por zonas (ground truth)
for i in range(len(df)):
    color = '#ffcccc' if df.iloc[i]['ground_truth'] == 1 else '#ccffcc'
    ax.axvspan(i - 0.5, i + 0.5, alpha=0.3, color=color, linewidth=0)

# Predicciones del IDS
colors = []
for _, row in df.iterrows():
    if row['ground_truth'] == 1 and row['alerta'] == 1:
        colors.append('#dc2626')  # TP - rojo
    elif row['ground_truth'] == 1 and row['alerta'] == 0:
        colors.append('#f59e0b')  # FN - amarillo
    elif row['ground_truth'] == 0 and row['alerta'] == 1:
        colors.append('#f97316')  # FP - naranja
    else:
        colors.append('#16a34a')  # TN - verde

ax.scatter(range(len(df)), df['alerta'], c=colors, s=8, alpha=0.7, zorder=2)
...
ax.set_title('Timeline de Detecciones — IDS Evil Twin (ML Ventana)', fontsize=13, fontweight='bold')
ax.set_yticks([0, 1])
ax.set_yticklabels(['Normal', 'Ataque'])
ax.grid(axis='y', alpha=0.3)

plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, 'timeline_detecciones_eviltwin.png'), dpi=200)
plt.show()
print(f"\n📁 Gráfica guardada: timeline_detecciones_eviltwin.png")
...
ax.set_title('Matriz de Confusión — IDS Evil Twin', fontsize=13, fontweight='bold')

plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, 'confusion_matrix_eviltwin.png'), dpi=200)
plt.show()
print(f"📁 Gráfica guardada: confusion_matrix_eviltwin.png")
...
# Histograma tiempos en modo ataque
if len(ataque) > 0:
...
# Histograma tiempos en modo normal
if len(normal) > 0:
...
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, 'tiempos_deteccion_eviltwin.png'), dpi=200)
plt.show()
print(f"📁 Gráfica guardada: tiempos_deteccion_eviltwin.png")

# =============================================================================
#          GRÁFICA 4: BSSIDs falsos por ventana (solo ataque)
# =============================================================================

if 'bssid_falsos' in df.columns:
    fig, ax = plt.subplots(figsize=(14, 4))
    
    ataque_idx = df[df['ground_truth'] == 1].index
    normal_idx = df[df['ground_truth'] == 0].index
    
    ax.bar(normal_idx, df.loc[normal_idx, 'bssid_falsos'], color='#16a34a', alpha=0.6, label='Normal', width=1)
    ax.bar(ataque_idx, df.loc[ataque_idx, 'bssid_falsos'], color='#dc2626', alpha=0.6, label='Ataque', width=1)
    
    ax.set_xlabel('Ventana #', fontsize=11)
    ax.set_ylabel('Paquetes BSSID falso', fontsize=11)
    ax.set_title('Paquetes de BSSID Falso por Ventana', fontsize=13, fontweight='bold')
    ax.legend(fontsize=10)
    ax.grid(alpha=0.3)
    
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, 'bssid_falsos_eviltwin.png'), dpi=200)
    plt.show()
    print(f"📁 Gráfica guardada: bssid_falsos_eviltwin.png")
...
print("="*60)
print(f"""
  Modelo: Random Forest (ventana de 50 paquetes)
  Dataset de entrenamiento: Tráfico real capturado en laboratorio
  Evaluación: {len(df)} ventanas en tiempo real
    - {(df['ground_truth'] == 0).sum()} ventanas de tráfico normal
    - {(df['ground_truth'] == 1).sum()} ventanas durante ataque Evil Twin
